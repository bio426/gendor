...
watch(() => pagination.page, () => getRows())
const search = ref("")
const rows = ref<tWorkshop.OrderSimple[]>([])

async function getRows() {
    loading.value = true
    const res = await workshopService.list({ search: search.value, page: pagination.page })
...
async function printOrder(id: number) {
    ...
    const total = res.items.reduce((acc, item) => item.price * item.quantity + acc, 0)
...
<template>
    <div class="relative min-h-screen w-full">
        <div class="w-11/12 mx-auto">
            ...
            <div class="mb-4">
                <router-link class="btn btn-primary" :to="{ name: 'workshop-create' }">Create</router-link>
            </div>
            <div class="mb-4">
                <div class="join w-full">
                    <input class="input input-bordered join-item flex-grow" placeholder="Search plate" v-model="search"
                        @keypress.enter="getRows" />
                    <button class="btn join-item" @click="getRows">Search</button>
                </div>
            </div>
            <Overlay :show="loading">
                <div class="mb-2 text-secondary text-sm">
                    Showing {{ pagination.from }} to {{ pagination.to }} of {{ pagination.total }} rows
                </div>
                            ...
                            <tr v-for="row in rows" :key="row.id">
                                ...
                                <td>
                                    <div class="flex gap-2">
                                        <button class="btn btn-xs btn-square" title="Print" @click="printOrder(row.id)">
                                            <PrinterIcon class="w-4 y-4" />
                                        </button>
                                        <button class="btn btn-xs btn-square" title="Copy"
                ...
                <div class="mb-8">
                    <Pagination :total="pagination.total" :count="20" v-model="pagination.page" />
                </div>
            </Overlay>
        </div>
    </div>
